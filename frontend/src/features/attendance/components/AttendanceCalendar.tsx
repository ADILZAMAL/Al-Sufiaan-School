...
export default function AttendanceCalendar({
  ...
  const showBoarding = hostel || dayboarding || !!calendar?.summary.hostel || !!calendar?.summary.dayboarding;
  const boardingLabel = hostel ? 'Hostel' : dayboarding ? 'Dayboard' : 'Boarding';
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      {!studentActive && (
        <div className="mb-4 flex items-center gap-2 rounded-lg bg-gray-50 border border-gray-200 px-4 py-2 text-sm text-gray-600">
          <span>Student has left school — showing historical attendance.</span>
        </div>
      )}
            ...
            >
              ...
              {isHoliday ? (
                ...
                <div className="space-y-0.5">
                  ...
                  {showBoarding && (records.hostel || records.dayboarding) && statusBadge(
                    records.hostel || records.dayboarding,
                    records.hostel ? 'Hst' : 'Day'
                  )}
                </div>
              )}
            </div>
