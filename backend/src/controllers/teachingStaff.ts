...
export const deleteTeachingStaff = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        ...
        await teachingStaff.destroy();

        return sendSuccess(res, null, 'Teaching staff deleted successfully');
    } catch (error: any) {
